
# Hi, I’m Adarsh 👋

I’m a **Junior Python Developer** with a strong interest in machine learning and AI.  
I enjoy building practical Python applications — from automation scripts and data analysis to ML and computer vision projects.

## 🔧 Tech Stack
- **Languages:** Python
- **Libraries:** Pandas, NumPy, Matplotlib, Scikit-learn, PyTorch
- **Computer Vision:** OpenCV, YOLO (ONNX)
- **Deployment & Tools:** ONNX, ONNX Runtime, Git, Linux/macOS

## 📌 Featured Projects
### 🔹 Edge AI Optimization
- Trained a CNN and exported it to ONNX
- Applied INT8 quantization and reduced model size by ~72%
- Benchmarked CPU inference performance  
👉 Repo: `edge-ai-optimization`

### 🔹 Real-Time Object Detection & Tracking
- Built a Python + OpenCV video pipeline
- Integrated YOLO (ONNX) with object tracking
👉 Repo: `opencv-real-time-detection`

### 🔹 Python Automation & CLI Tools
- File automation, CSV processing, command-line tools
👉 Repos: `python-file-automation`, `python-cli-tool`

### 🔹 Python Data Analysis
- Data cleaning and visualisation using Pandas & Matplotlib
👉 Repo: `python-data-analysis-basics`

## 📫 Get in touch
- LinkedIn: https://www.linkedin.com/in/adarsh-ravi2121/
